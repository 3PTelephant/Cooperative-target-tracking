...
std_msgs::Empty land;
geometry_msgs::Twist bp1_cmd_vel;
...
void target_vrpn(const cotrack::drone_state::ConstPtr& msg6)
{
target_state=*msg6;
...
{   ros::init(argc, argv, "bebop_test1");
    ros::NodeHandle nh("~");


    //------------控制部分的topic订阅------------------
    ros::Publisher bp1_takeoff_pub= nh.advertise<std_msgs::Empty>("/bebop1/takeoff", 5);         // 发布 起飞命令
    ros::Publisher bp1_land_pub   = nh.advertise<std_msgs::Empty>("/bebop1/land", 5);            // 发布 降落命令
    ros::Publisher bp1_cmd_pub    = nh.advertise<geometry_msgs::Twist>("/bebop1/cmd_vel", 5);    // 发布 移动命令
...
//        cout<<"x_t,y_t:"<<x_t<<","<<y_t<<endl;
        ...
        ros::spinOnce();
        //一号机为右侧的
        u_abs[0]=Kr1*(h_x_1+x_t-bp1_state.pos_x)+Kv1*(dot_h_x_1+dot_x_t-bp1_state.vel_x)+dot2_h_x_1+dot2_x_t;
        u_abs[1]=Kr1*(h_y_1+y_t-bp1_state.pos_y)+Kv1*(dot_h_y_1+dot_y_t-bp1_state.vel_y)+dot2_h_y_1+dot2_y_t;
        u_cooper[0]=K_coop_r*((bp4_state.pos_x-h_x_4)-(bp1_state.pos_x-h_x_1))+K_coop_v*((bp4_state.vel_x-dot_h_x_4)-(bp1_state.vel_x-dot_h_x_1));
        u_cooper[1]=K_coop_r*((bp4_state.pos_y-h_y_4)-(bp1_state.pos_y-h_y_1))+K_coop_v*((bp4_state.vel_y-dot_h_y_4)-(bp1_state.vel_y-dot_h_y_1));
//        u_cooper[0]=0;
//        u_cooper[1]=0;
        u_abs=u_abs+u_cooper;

        Rotation_R2B << cos(bp1_state.yaw), sin(bp1_state.yaw), -sin(bp1_state.yaw), cos(bp1_state.yaw);
        u_abs = Rotation_R2B * u_abs;
        cout<<"u_abs"<<u_abs<<endl;
        bp1_cmd_vel.linear.x = asin(u_abs[0]/9.8)*9/pi;
        bp1_cmd_vel.linear.y = asin(u_abs[1]/9.8)*9/pi;
        bp1_cmd_vel.linear.z = 0;

        if (bp1_target_yaw-bp1_state.yaw>pi)
        bp1_cmd_vel.angular.z=(bp1_target_yaw-bp1_state.yaw-2*pi)*0.5+1.8*w/pi;
        else if (bp1_target_yaw-bp1_state.yaw<-pi)
            bp1_cmd_vel.angular.z=(bp1_target_yaw-bp1_state.yaw+2*pi)*0.5+1.8*w/pi;
            else
            bp1_cmd_vel.angular.z=(bp1_target_yaw-bp1_state.yaw)*0.5+1.8*w/pi;

        if(abs(bp1_state.pos_x)>3||abs(bp1_state.pos_y)>3)
        {bp1_cmd_vel.linear.x=0;
          bp1_cmd_vel.linear.y=0;
        }
        bp1_cmd_pub.publish(bp1_cmd_vel);
        break;
