...
{   ros::init(argc, argv, "bebop_test3");
    ...
    while ( ros::ok()&&t<30 )
    {
      ...
      if (fly_mode==2)
      {
        ...
        //一号机为右侧的
        u_abs[0]=Kr1*(h_x_3+x_t-bp3_state.pos_x)+Kv1*(dot_h_x_3-bp3_state.vel_x)+dot2_h_x_3;
        u_abs[1]=Kr1*(h_y_3+y_t-bp3_state.pos_y)+Kv1*(dot_h_y_3-bp3_state.vel_y)+dot2_h_y_3;
        u_cooper[0]=K_coop_r*((bp4_state.pos_x-h_x_4)-(bp3_state.pos_x-h_x_3))+K_coop_v*((bp4_state.vel_x-dot_h_x_4)-(bp3_state.vel_x-dot_h_x_3));
        u_cooper[1]=K_coop_r*((bp4_state.pos_y-h_y_4)-(bp3_state.pos_y-h_y_3))+K_coop_v*((bp4_state.vel_y-dot_h_y_4)-(bp3_state.vel_y-dot_h_y_3));
//        u_cooper[0]=0;
//        u_cooper[1]=0;
        ...
        if(abs(bp3_state.pos_x)>2.5||abs(bp3_state.pos_y)>2.5)
