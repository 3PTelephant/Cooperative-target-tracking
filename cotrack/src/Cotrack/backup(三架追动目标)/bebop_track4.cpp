...
{   ros::init(argc, argv, "bebop_test4");
    ...
    while ( ros::ok()&&t<30 )
    {
      ...
      switch(fly_mode)
      {
        ...
        x_t=-1+r_target*cos(w_target*t);
        y_t=-1+r_target*sin(w_target*t);
        ...
        h_x_3=r*cos(w*t+2*pi/3);
        h_y_3=r*sin(w*t+2*pi/3);
        dot_h_x_3=-r*w*sin(w*t+2*pi/3);
        dot_h_y_3=r*w*cos(w*t+2*pi/3);
        dot2_h_x_3=-r*w*w*cos(w*t+2*pi/3);
        dot2_h_y_3=-r*w*w*sin(w*t+2*pi/3);

        cout<<"statemachine="<<fly_mode<<endl;
        ros::spinOnce();
        //一号机为右侧的
        u_abs[0]=Kr1*(h_x_4+x_t-bp4_state.pos_x)+Kv1*(dot_h_x_4+dot_x_t-bp4_state.vel_x)+dot2_h_x_4+dot2_x_t;
        u_abs[1]=Kr1*(h_y_4+y_t-bp4_state.pos_y)+Kv1*(dot_h_y_4+dot_y_t-bp4_state.vel_y)+dot2_h_y_4+dot2_y_t;
