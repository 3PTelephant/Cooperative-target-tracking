...
{   ros::init(argc, argv, "bebop_test2");
    ...
    while ( ros::ok()&&t<30 )
    {
      ...
      switch(fly_mode)
      {
        ...
        ros::spinOnce();
        //一号机为右侧的
        u_abs[0]=Kr1*(h_x_2+x_t-bp2_state.pos_x)+Kv1*(dot_h_x_2-bp2_state.vel_x)+dot2_h_x_2;
        u_abs[1]=Kr1*(h_y_2+y_t-bp2_state.pos_y)+Kv1*(dot_h_y_2-bp2_state.vel_y)+dot2_h_y_2;
