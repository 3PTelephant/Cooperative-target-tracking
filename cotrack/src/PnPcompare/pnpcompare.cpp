...
double time4HOMO=0.0;
double num4HOMO=0;
double time4RPnP=0.0;
double num4RPnP=0.0;
double time4RPP=0.0;
double num4RPP=0.0;
// 四元数转Euler
...
int main(int argc, char **argv)
{
    ...
    camera_matrix.ptr<double>(0)[0]=588;
    camera_matrix.ptr<double>(0)[2]=319.760522;
    camera_matrix.ptr<double>(1)[1]=589;
    camera_matrix.ptr<double>(1)[2]=225.057360;
    camera_matrix.ptr<double>(2)[2]=1.0f;

    distortion_coefficients=cv::Mat(5,1,CV_64FC1,cv::Scalar::all(0));
    distortion_coefficients.ptr<double>(0)[0]=-0.416205;
    distortion_coefficients.ptr<double>(1)[0]=0.196962;
    distortion_coefficients.ptr<double>(2)[0]=0.000460;
    distortion_coefficients.ptr<double>(3)[0]=-0.001343;
    ...
    Mat P=cv::Mat(3,3,CV_64FC1,cv::Scalar::all(0));
    P.ptr<double>(0)[0]=510.013062;
    P.ptr<double>(0)[2]=317.999006;
    P.ptr<double>(1)[1]=545.944275;
    P.ptr<double>(1)[2]=222.786603;
    P.ptr<double>(2)[2]=1.0f;
    ...
    else
    {
        ...
        out_data_file <<" Time "<< " RPNP.x " << " RPNP.y "<< " RPNP.z " \
                                << " HOMO.x " << " HOMO.y "<< " HOMO.z " \
                                << " RPP.x " << " RPP.y "<< " RPP.z " \
                                <<std::endl;
    }
    ...
    capture.open("/home/jacky/cotrack_ws/4445.avi");
    int total_frame=capture.get(CV_CAP_PROP_FRAME_COUNT);
//    int total_frame=460;
    int frame_num=1;
    //----------------------------------------主循环------------------------------------
    while(frame_num<total_frame&&ros::ok())
    {
        clock_t start=clock();
        capture>>img;
        ros::spinOnce();
...
//        cout<<"point4"<<markerCorners[0][3].x<<endl;
        ...
        if (markerids.size()>0)
        {
                ...
                clock_t homo_start=clock();
                cv::aruco::estimatePoseSingleMarkers(markerCorners,0.20,camera_matrix,distortion_coefficients,rvec,tvec);
                clock_t homo_stop=clock();
                cout<<"homo_time"<<homo_stop-homo_start<<endl;
                time4HOMO=time4HOMO+(homo_stop-homo_start)/1000.0;
                num4HOMO+=1;
                ...
                Point3f HOMO_POS=Position_OcInW;
                ...
                undistortPoints(markerCorners[0],corner,camera_matrix,distortion_coefficients,P);

//                undistortPoints(markerCorners[0],corner,camera_matrix,distortion_coefficients,P);
                cout<<"corner"<<corner<<endl;
                Imgpts.clear();
                Imgpts.push_back(Point2d(corner[0]));
                Imgpts.push_back(corner[1]);
                Imgpts.push_back(corner[2]);
                Imgpts.push_back(corner[3]);

                Point2d PrincipalPoint;
                double focallength=530;

                PrincipalPoint.x=317.999006;
                PrincipalPoint.y=222.786603;
                 Point3f RPNP_POS;
                 clock_t rpnp_start=clock();
                if(RPnP(WorldPts,Imgpts,ALLrotM,ALLTransM,PrincipalPoint,focallength))
              {
                  clock_t rpnp_stop=clock();
                  cout<<"rpnp_time"<<rpnp_stop-rpnp_start<<endl;
                  time4RPnP+=(rpnp_stop-rpnp_start)/1000.0;
                  num4RPnP+=1;
                Mat tmpr;
                double rm[9];
                tmpr = cv::Mat(3, 3, CV_64FC1, rm);
                Rodrigues(ALLrotM,tmpr);
                LookMat(tmpr,"Gotten R");
                LookMat(ALLTransM,"Gotten T");
                 r11 = tmpr.ptr<double>(0)[0];
                 r12 = tmpr.ptr<double>(0)[1];
                 r13 = tmpr.ptr<double>(0)[2];
                 r21 = tmpr.ptr<double>(1)[0];
                 r22 = tmpr.ptr<double>(1)[1];
                 r23 = tmpr.ptr<double>(1)[2];
                 r31 = tmpr.ptr<double>(2)[0];
                 r32 = tmpr.ptr<double>(2)[1];
                 r33 = tmpr.ptr<double>(2)[2];
                ...
                Theta_W2C.z = -1 * thetaz;
                //偏移向量
                 x = ALLTransM.ptr<double>(0)[0], y =ALLTransM.ptr<double>(1)[0], z = ALLTransM.ptr<double>(2)[0];
                 cout<<"x"<<x<<endl;
                 cout<<"y"<<y<<endl;
                 cout<<"z"<<z<<endl;
                ...
                Position_OcInW.y = y*-1;
                Position_OcInW.z = z*-1;
              RPNP_POS =Position_OcInW;
              }
                else
                    printf("The pose can not be calculated by RPnP");

              //RPP算法
                undistortPoints(markerCorners[0],corner,camera_matrix,distortion_coefficients);

                Mat model = Mat::zeros(3, 4, CV_64F); // 3D points, z is zero
                Mat iprts = Mat::ones(3, 4, CV_64F); // 2D points, homogenous points
                Mat rotation;
                Mat translation;
                int iterations;
                double obj_err;
                ...
                model.at<double>(0,0) = -0.1;
                model.at<double>(1,0) = 0.1;
                model.at<double>(0,1) = 0.1;
                model.at<double>(1,1) = 0.1;
                model.at<double>(0,2) = 0.1;
                model.at<double>(1,2) = -0.1;
                model.at<double>(0,3) = -0.1;
                model.at<double>(1,3) = -0.1;

                iprts.at<double>(0,0) = corner[0].x;
                iprts.at<double>(1,0) = corner[0].y;
                iprts.at<double>(0,1) = corner[1].x;
                iprts.at<double>(1,1) = corner[1].y;
                iprts.at<double>(0,2) = corner[2].x;
                iprts.at<double>(1,2) = corner[2].y;
                iprts.at<double>(0,3) = corner[3].x;
                iprts.at<double>(1,3) = corner[3].y;
                clock_t rpp_start=clock();
                ...
                clock_t rpp_stop=clock();
                cout<<"RPP_time"<<rpp_stop-rpp_start<<endl;
                time4RPP+=(rpp_stop-rpp_start)/1000.0;
                num4RPP+=1;
                printf("\n");
                  ...
                  printf("Image error: %f\n", img_err);
                   ...
                   r11 = rotation.ptr<double>(0)[0];
                   r12 = rotation.ptr<double>(0)[1];
                   r13 = rotation.ptr<double>(0)[2];
                   r21 = rotation.ptr<double>(1)[0];
                   r22 = rotation.ptr<double>(1)[1];
                   r23 = rotation.ptr<double>(1)[2];
                   r31 = rotation.ptr<double>(2)[0];
                   r32 = rotation.ptr<double>(2)[1];
                   r33 = rotation.ptr<double>(2)[2];
                  ...
                  Theta_W2C.z = -1 * thetaz;
                  //偏移向量
                   x = translation.ptr<double>(0)[0], y =translation.ptr<double>(1)[0], z = translation.ptr<double>(2)[0];
                   cout<<"x"<<x<<endl;
                   cout<<"y"<<y<<endl;
                   cout<<"z"<<z<<endl;
                  ...
                  Position_OcInW.y = y*-1;
                  Position_OcInW.z = z*-1;
                  Point3f RPP_POS=Position_OcInW;



                cur_time = get_dt(begin_time);
                photo_time =cur_time;
                out_data_file << cur_time <<"  "<< RPNP_POS.x <<"  "<< RPNP_POS.y <<"  "<<RPNP_POS.z<<" "\
                                                << HOMO_POS.x <<"  "<< HOMO_POS.y <<"  "<<HOMO_POS.z<<" "\
                                                << RPP_POS.x <<"  "<< RPP_POS.y <<"  "<<RPP_POS.z<<" "\
    ...
    cout<<"HOMO_MEANTIME="<<time4HOMO/num4HOMO<<endl;
    cout<<"RPP_MEANTIME="<<time4RPP/num4RPP<<endl;
    cout<<"RPNP_MEANTIME="<<time4RPnP/num4RPnP<<endl;
    cout<<"NUM_HOMO"<<num4HOMO<<endl;
    cout<<"NUM_RPP"<<num4RPP<<endl;
    cout<<"NUM_RPNP"<<num4RPnP<<endl;
    cout<<"time_HOMO"<<time4HOMO<<endl;
    cout<<"time_RPP"<<time4RPP<<endl;
    cout<<"time_RPNP"<<time4RPnP<<endl;
